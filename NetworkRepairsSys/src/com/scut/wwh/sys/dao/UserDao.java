...
//		System.out.println("rs.next----"+rs.next());
	...
	public ResultSet userList(Connection con,PageBean pageBean,User user) throws Exception{
		StringBuffer sb=new StringBuffer("select * from t_user where level in ('管理员','职员') and userName not in('wlzxroot')");
		//如果姓名不为空，则查询满足该姓名的记录，其他为空
		if(StringUtil.isNotEmpty(user.getName()) && StringUtil.isEmpty(user.getUserName()) && StringUtil.isEmpty(user.getSex())){
			sb.append(" and name like '%"+user.getName()+"%'");
		}
		//如果账号不为空，则查询满足该 账号的记录，其他为空
		if(StringUtil.isNotEmpty(user.getUserName()) && StringUtil.isEmpty(user.getName()) && StringUtil.isEmpty(user.getSex())){
			sb.append(" and userName like '%"+user.getUserName()+"%'");
		}
		//如果性别不为空，则查询满足该性别的记录，其他为空
		if(StringUtil.isNotEmpty(user.getSex()) && StringUtil.isEmpty(user.getUserName()) && StringUtil.isEmpty(user.getName())){
			sb.append(" and sex like '%"+user.getSex()+"%'");
		}
		//如果姓名和账户都不为空的时候，性别为空
		if(StringUtil.isNotEmpty(user.getName()) && StringUtil.isNotEmpty(user.getUserName()) && StringUtil.isEmpty(user.getSex())){
			sb.append(" and name like '%"+user.getName()+"%' and userName like'%"+user.getUserName()+"%'");
		}
		//如果姓名和性别都不为空，账户为空
		if(StringUtil.isNotEmpty(user.getName()) && StringUtil.isNotEmpty(user.getSex()) && StringUtil.isEmpty(user.getUserName())){
			sb.append(" and name like '%"+user.getName()+"%' and sex like '%"+user.getSex()+"%'");
		}
		//如果账户和性别都不为空，姓名为空
		if(StringUtil.isNotEmpty(user.getUserName()) && StringUtil.isNotEmpty(user.getSex()) && StringUtil.isEmpty(user.getName())){
			sb.append(" and userName like '%"+user.getUserName()+"%' and sex like '%"+user.getSex()+"%'");
		}
		//如果姓名，账户，性别都不为空
		if(StringUtil.isNotEmpty(user.getUserName()) && StringUtil.isNotEmpty(user.getSex()) && StringUtil.isNotEmpty(user.getName())){
			sb.append(" and userName like '%"+user.getUserName()+"%' and sex like '%"+user.getSex()+"%' and name like '%"+user.getName()+"%'");
		}
		if(pageBean!=null){
	...
	public int userCount(Connection con,User user)throws Exception{
		StringBuffer sb= new StringBuffer("select count(*) as total from t_user where level in('管理员','职员') and userName not in('wlzxroot')");
		//如果姓名不为空，则查询满足该姓名的记录，其他为空
		if(StringUtil.isNotEmpty(user.getName()) && StringUtil.isNotEmpty(user.getUserName()) && StringUtil.isEmpty(user.getSex())){
			sb.append(" and name like '%"+user.getName()+"%'");
		}
